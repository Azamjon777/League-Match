...
import petto.para.leaguematch.databinding.FragmentFirstEcuadorBinding

class FirstEcuadorFragment : Fragment() {
    private var _binding: FragmentFirstEcuadorBinding? = null
    ...
    override fun onCreateView(
        ...
        _binding = FragmentFirstEcuadorBinding.inflate(inflater, container, false)
        return binding.root
    }
        ...
        binding.tvFindOut.setOnClickListener {
            if (isButtonClickable) { // Проверяем доступность кнопки
                ...
                CoroutineScope(Dispatchers.Main).launch {
                    delay(2000)
                    findNavController().navigate(R.id.action_firstEcuadorFragment_to_forEcuadorFragment)
                }
            }
        }
    }
